% Taylor Tallerday
function designSenior(GST, L, longitude, H, rho, El, Az, LST, rho_dot, El_dot, Az_dot, TOF, siteName)
    ...
    displayForSiteAndSatellite (GST, longitude, H, rho, El, Az, L, LST, rho_dot, El_dot, Az_dot);
    findingVectors(GST, L, longitude, H, rho, El, Az, LST, rho_dot, El_dot, Az_dot, TOF);
    fclose(fid);
...
function orbitCalculations(GST, L, longitude, H, R_vector, V_vector, TOF)
    ...
    i_vector = [1, 0, 0];
        ...
        %% CHANGED LOCATION OF RAAN and AOP to be calculated here
...
function displayForSiteAndSatellite (GST, longitude, H, rho, El, Az, L, LST, rho_dot, El_dot, Az_dot)
    ...
    Az = Az/(pi/180);
    El = El/(pi/180);
    fprintf(fid,"Inital Range tracking site location is at: \n");
    fprintf(fid, "Latitude = %.4f degrees \n", L);
    fprintf(fid, "Longitude = %.4f degrees east \n", longitude);
    fprintf(fid, "Altitude above sea level = %.4f km \n", H);
    fprintf(fid, "Greenwich Sidereal Time = %.4f degrees \n", GST);
    fprintf(fid, "Local Sidereal Time = %.4f degrees \n\n", LST);
    fprintf(fid, "Satellite is located at: \n");
    fprintf(fid, "Range = %.4f km \n", rho);
    fprintf(fid, "Azimuth = %.4f degrees \n", Az);
    fprintf(fid, "Elevation = %.4f degrees \n\n", El);
    fprintf(fid, "Satellite is moving at: \n");
    fprintf(fid, "Range rate of change = %.4f km/sec\n", rho_dot);
    fprintf(fid, "Azimuth rate of change = %.4f rad/sec\n", Az_dot);
    fprintf(fid, "Elevation rate of change = %.4f rad/sec\n\n\n", El_dot);
end
...
function updatedVectors(L, longitude, H, GST, TOF, a, E, FTA, i, RAAN, AOP, mu)
    ...
    R_Vector_2 = T\R_pqw;
    V_Vector_2 = T\V_pqw;
    fprintf(fid, "Final Range Tracking Site Location at: \n");
...
checkingStations(Az_final, El_final)
end
...
function trackingStationsAvailable(nameStation, latitude, longitude, elevation)
    ...
    fprintf(fid, "Latitude for tracking station: %.4f \n", latitude);
    fprintf(fid, "Longitude for tracking station: %.4f \n", longitude);
    fprintf(fid, "Altitude above sea level: %.4f \n\n", elevation);
end
...
% the satellite
function checkingStations(Az_final, El_final)
    fid = fopen('output.txt', 'a');
...
%{
    % Checking if the longitude of the satellite is within 20 degress of
    % tracking station
    if longitude_station >= rho_final - 20 && longitude_station <= rho_final
        if latitude_station >=
            fprintf("%s is within range of the satellite", nameStation)
        end
    end
end
